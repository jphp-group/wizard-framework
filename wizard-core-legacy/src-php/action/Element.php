...
class Element
{
    static function setValue($object, $value)
    {
        ...
        if ($object instanceof UISelectControl) {
            $object->selected = $value;
            return true;
        }

        if ($object instanceof UIListView) {
    ...
    static function setText($object, $value)
    {
        ...
        if ($object instanceof UILabeled || $object instanceof UITextInputControl) {
            $object->text = $value;
            return true;
        } elseif ($object instanceof UISelectControl) {
            $object->selected = $value;
            return true;
        } elseif ($object instanceof UIListView) {
            $object->items->clear();
            $object->items->addAll(Str::split($value, "\n"));
            return true;
        } elseif ($object instanceof UIForm || $object instanceof UIWindow) {
            $object->title = $value;
            return true;
    ...
    static function appendText($object, $value)
    {
        ...
        if ($object instanceof UILabeled || $object instanceof UITextInputControl) {
            $object->text .= $value;
            return true;
        } else if ($object instanceof UIListView || $object instanceof UISelectControl) {
            $object->items = $object->items + [$value];
            return true;
        } else if ($object instanceof UIForm || $object instanceof UIWindow) {
            $object->title .= $value;
            return true;
