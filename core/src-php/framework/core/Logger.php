...
class Logger
{
    ...
    protected static $format = "{level} [{context}] ({time}) {message}";
    ...
    public static function stdoutWriter(bool $withColor = false): callable
    {
        ...
        $format = function ($str, $color) use ($ANSI_CODES) {
            $color_attrs = explode("+", $color);
            $ansi_str = "";

            foreach ($color_attrs as $attr) {
                $ansi_str .= char::of(27) . "[" . $ANSI_CODES[$attr] . "m";
            }

            $ansi_str .= $str . char::of(27) . "[" . $ANSI_CODES["off"] . "m";
            return $ansi_str;
        };

        return function ($type, $message) use ($stdout, $format, $withColor, $LEVEL_COLORS) {
            if ($withColor) {
