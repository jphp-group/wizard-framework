 ...
 * @packages httpclient
 */
class HttpDownloader extends Component
{
     ...
     */
    public function download(string $url, string $fileName = null): HttpResponse
    {
        $this->checkBreak();
        ...
        if ($response->isSuccess()) {
            ...
            uiLater(function () use ($contentLength, $url, $fileName, $response) {
                $this->trigger(new Event(
                    'progress', $this, null, ['url' => $url, 'response' => $response, 'file' => $fileName, 'max' => $contentLength, 'progress' => 0]
                ));
            });
            ...
            try {
                fs::copy($stream, $this->useTempFile ? $fileName . self::TEMP_EXT : $fileName, function ($progress, $bytes) use ($contentLength, $url, $fileName, $response) {
                    ...
                    $this->trigger(new Event(
                        'progress', $this, null, ['url' => $url, 'response' => $response, 'file' => $fileName, 'max' => $contentLength, 'progress' => $progress]
                    ));
                }, $this->bufferSize);
            } catch (IOException $e) {
                $this->checkBreak();
                ...
                $this->trigger(new Event('errorOne', $this, null, ['url' => $url, 'file' => $fileName, 'response' => $response, 'error' => $e->getMessage()]));
                return $response;
            }
            ...
            if ($this->useTempFile) {
                ...
                if (!File::of($fileName . self::TEMP_EXT)->renameTo($fileName)) {
                    ...
                    $this->trigger(new Event(
                        ...
                        $this,
                        null,
                        ['url' => $url, 'file' => $fileName, 'response' => $response, 'error' => 'Cannot rename file']
                    ));
                }
            ...
            $this->trigger(new Event('successOne', $this, null, ['url' => $url, 'file' => $fileName, 'response' => $response]));
        } else {
            $this->checkBreak();
            $this->trigger(new Event('errorOne', $this, null, ['url' => $url, 'file' => $fileName, 'response' => $response]));
        }
     ...
     * @param $value
     */
    protected function setStatValue($url, string $name, $value)
    {
     ...
     * @return float
     */
    public function getBitSpeed(): float
    {
    ...
    public function start()
    {
        ...
        $countDone = new SharedValue(null);
                ...
                if ($countDone->setAndGet(function ($v) { return $v+1; }) == sizeof($urls)) {
                    if (!$this->_break) {
                        if ($this->_failedFiles->isEmpty()) {
                            $this->trigger(new Event('successAll', $this));
                        }
                    } else {
                        // ...
                    }

                    $this->trigger(new Event('done', $this));
     ...
     * @return int
     */
    public function getUrlSize($url): int
    {
     ...
     * @return float|int
     */
    public function getUrlProgress($url): float
    {
     ...
     * @return bool
     */
    public function isUrlSuccess($url): bool
    {
     ...
     * @return bool
     */
    public function isUrlWaiting($url): bool
    {
     ...
     * @return bool
     */
    public function isUrlDownloading($url): bool
    {
     ...
     * @return bool
     */
    public function isUrlError($url): bool
    {
     ...
     * @return array
     */
    public function getDownloadedUrls(): array
    {
     ...
     * @return array
     */
    public function getProgressUrls(): array
    {
    ...
    private static function textToArray($text, $trimValues = false)
    {
