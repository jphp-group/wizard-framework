...
class HttpClient extends Component
{
    ...
    public function __construct()
    {
        ...
        $this->_lock = new SharedValue(null);
    }
    ...
    public function execute($url, $method = 'GET', $data = null)
    {
        ...
        $this->_lock->synchronize(function () use (&$connect, &$body, $url, $method, $data) {
            ...
            switch ($method) {
                ...
                default:
                    $data = flow((array)$this->data, (array)$data)->toMap();
            ...
            if ($existsBody) {
                switch (str::upper($this->requestType)) {
                    case 'NONE':
                        break;
    ...
    protected function connect(URLConnection $connection, $body)
    {
        ...
        if ($response->isSuccess()) {
            $this->trigger(new Event('success', $this, null, ['response' => $response]));
        }

        if ($response->isFail()) {
            $this->trigger(new Event('error', $this, null, ['response' => $response]));
        }

        if ($response->isServerError()) {
            $this->trigger(new Event('errorServer', $this, null, ['response' => $response]));
        }

        if ($response->isNotFound()) {
            $this->trigger(new Event('errorNotFound', $this, null, ['response' => $response]));
        }

        if ($response->isAccessDenied()) {
            $this->trigger(new Event('errorAccessDenied', $this, null, ['response' => $response]));
        }
